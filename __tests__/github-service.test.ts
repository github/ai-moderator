...
describe('GitHub Service Functions', () => {
  ...
  describe('addLabels', () => {
    it('should add labels to an issue', async () => {
      ...
      await addLabels(mockOctokit as any, mockContext as any, issueNumber, labels);
  ...
  describe('minimizeComment', () => {
    ...
    mutation ($nodeId: ID!) {
      ...
      expect(mockOctokit.graphql).toHaveBeenCalledWith(expectedQuery, { nodeId });
    });
    ...
    it('should handle GraphQL errors', async () => {
      ...
      await expect(
        minimizeComment(mockOctokit as any, nodeId)
